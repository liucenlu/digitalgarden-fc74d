---
{"dg-publish":true,"permalink":"/前端八股/浏览器/前端面试 浏览器原理篇_w3cschool/","created":"2025-06-02T22:00:06.000+08:00","updated":"2025-06-17T21:02:19.805+08:00"}
---
...
![](/img/user/前端八股/浏览器/Attachments/1664177172577389.png)  
...
- **DOM2 级事件模型**，在该事件模型中，一次事件共有三个过程（==事件捕获-事件处理-事件冒泡==），第一个过程是事件捕获阶段。捕获指的是事件从 document 一直向下传播到目标元素，依次检查经过的节点是否绑定了事件监听函数，如果有则执行。后面两个阶段和 IE 事件模型的两个阶段相同。这种事件模型，**事件绑定的函数是addEventListener，其中第三个参数可以指定事件是否在捕获阶段执行。**
...
**事件委托** 原理就是利用事件冒泡机制把里层所需要响应的事件绑定到外层

### 事件传播流程
DOM 事件传播是 “三阶段事件流” 模型（由 W3C 定义）：

1. 捕获阶段（eventPhase === 1）：事件从根节点向目标节点传播；
2. 目标阶段（eventPhase === 2）：事件到达目标节点；
3. 冒泡阶段（eventPhase === 3）：事件从目标节点向上返回根节点。
![Pasted image 20250601104351.png](/img/user/Pasted%20image%2020250601104351.png)
### 事件捕获

事件从DOM树的根节点开始，然后**逐级向下捕获**到最具体的元素（目标元素）。addEventListener方法的第三个参数是 true 的时候
...
![](https://cdn.nlark.com/yuque/0/2021/png/604921/1616298050249-9879333f-039c-4906-b089-9866f144978b.png)

addEventListener方法的第三个参数是 true 的时候，点击蓝色div，这时候为**捕获阶段**

![](https://cdn.nlark.com/yuque/0/2021/png/604921/1616298351744-91ec45ff-b189-44f3-88d1-fa93ba7b126f.png)

  
### 事件冒泡

就是指父元素和子元素有相同的事件，当触发子元素事件时，会向上冒泡，同时也会触发父元素事件。多个元素嵌套,有层次关系,这些元素都注册了相同的事件,如果里面的元素的事件触发了,外面的元素的该事件自动的触发了

> **事件冒泡的使用场景：** 事件冒泡的一个**常见应用场景是事件委托**（event delegation）。通过事件委托，我们可以将事件处理程序**绑定**到**父**级元素，而不是直接绑定到每个子元素，从而**减少了事件处理程序的数量**，提高了**性能**和代码的可**维护性**。
...
addEventListener方法的第三个参数是 false 的时候，点击蓝色div，这时候为**冒泡阶段**

![](https://cdn.nlark.com/yuque/0/2021/png/604921/1616298226639-fbfdce54-c987-4d7a-b47a-5bac033eb1d1.png)

## 阻止事件冒泡

- **event.stopPropagation(）**：在事件处理程序中调用该方法可立即停止事件的进一步传播。
- **event.cancelBubble**：把该属性设置为true (IE特有的,谷歌支持,火狐不支持)
- **event.preventDefault()**方法：该方法可以阻止事件的默认行为，并且也会停止事件冒泡。
- **return false**：如果事件处理程序返回false，则会阻止事件继续传播和执行默认行为。
- **addEventListener()**方法的capture 参数：当为addEventListener(）方法的capture参数设置为true时，事件会从上层元素被处理到当前元素。例如：element. addEventListener('click', eventHandler, true);
- **使用事件委托（Event delegation）**：将事件处理程序绑定到父元素而不是子元素上，然后通过event.target 属性判断事件的来源，可以选择性地阻止冒泡。
...
<script>
  document.getElementById("div1").addEventListener('click', function (event) {
    ...
    if (event.target == document.getElementById('div3')) {
      event.stopPropagation();
      console.log("事件委托里面 event.stopPropagation()阻止事件冒泡");
      //其他逻辑处理
    }
  })
</script>
...
![](/img/user/前端八股/浏览器/Attachments/1664177613180370.gif)  
...
![](/img/user/前端八股/浏览器/Attachments/1664177719711713.png)  
...
![](/img/user/前端八股/浏览器/Attachments/1664177736769496.png)  
...
![](/img/user/前端八股/浏览器/Attachments/1664177766768490.png)  
...
![](/img/user/前端八股/浏览器/Attachments/1664178092640833.png)  
