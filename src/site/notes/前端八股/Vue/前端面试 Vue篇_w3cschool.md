...
![](/img/user/前端八股/Vue/Attachments/1664430669601596.png)
...
==响应式系统（数据驱动视图）、模板编译（template->dom）、虚拟dom & diff算法、组件化开发、生命周期机制==

### 1. Vue的响应式原理

当一个Vue实例创建时，Vue会遍历data中的属性，用 Object.defineProperty（vue3.0使用proxy ）将它们转为 getter/setter，并且在内部追踪相关依赖，在属性被访问和修改时通知变化。 每个组件实例都有相应的 watcher 程序实例，它会在组件渲染的过程中把属性记录为依赖，之后当依赖项的setter被调用时，会通知watcher重新计算，从而致使它关联的组件得以更新。  

![](/img/user/前端八股/Vue/Attachments/1664430790819020.png)  
Vue 的 `render function` 是用 JavaScript 手动构建虚拟 DOM 的函数，它是 Vue 内部编译模板的结果，也可以由开发者手写用于更高自由度的 DOM 操作，是构建组件视图结构的核心方法之一。
### 2. 双向数据绑定的原理

> [!NOTE]
> **双向数据绑定**就是：  
> 数据变化会更新视图（数据驱动视图）  
> 视图变化（如用户输入）也会更新数据（视图驱动数据）
> 
> 👉 实现的是数据和视图的“同步”。

Vue.js 是采用**数据劫持**结合**发布者-订阅者模式**的方式，通过Object.defineProperty()来劫持各个属性的setter，getter，在数据变动时发布消息给订阅者，触发相应的监听回调。主要分为以下几个步骤：  

1. 监听器observer：对数据对象进行递归遍历，包括子属性对象的属性，都加上setter和getter，这样的话，给这个对象的某个值赋值，就会触发setter，那么就能监听到了数据变化
2. 解析器compile：解析模板指令，将模板中的变量替换成数据，然后初始化渲染页面视图，并将每个指令对应的节点绑定更新函数，添加监听数据的订阅者，一旦数据有变动，收到通知，更新视图
3. 订阅者Watcher：是Observer和Compile之间通信的桥梁，主要做的事情是: ①在自身实例化时往属性订阅器(dep)里面添加自己 ②自身必须有一个update()方法 ③待属性变动dep.notice()通知时，能调用自身的update()方法，并触发Compile中绑定的回调，则功成身退。
4. 订阅器Dep：采用发布-订阅的设计模式，用来收集订阅者watcher，对监听器Observer和订阅者watcher进行统一管理。
...
![](/img/user/前端八股/Vue/Attachments/1664430835300456.png)  
...
在 Vue3.0 中已经不使用这种方式了，而是通过使用 Proxy 对对象进行代理，从而实现数据劫持。使用Proxy 的好处是==它可以完美的监听到任何方式的数据改变==，唯一的缺点是兼容性的问题，因为 Proxy 是 ES6 的语法。
...
![](/img/user/前端八股/Vue/Attachments/1664430951359474.png)  
...
这种模式==实现了 Model和View的数据自动同步，因此开发者只需要专注于数据的维护操作即可，而不需要自己操作DOM。==

![](/img/user/前端八股/Vue/Attachments/1664431095994064.png)  

**（3）MVP**  

MVP 模式与 MVC 唯一不同的在于 Presenter 和 Controller。在 MVC 模式中使用观察者模式，来实现当 Model 层数据发生变化的时候，通知 View 层的更新。这样 View 层和 Model 层耦合在一起，当项目逻辑变得复杂的时候，可能会造成代码的混乱，并且可能会对代码的复用性造成一些问题。MVP 的模式通过使用 Presenter 来实现对 View 层和 Model 层的解耦。MVC 中的Controller 只知道 Model 的接口，因此它没有办法控制 View 层的更新，MVP 模式中，View 层的接口暴露给了 Presenter 因此可以在 Presenter 中将 Model 的变化和 View 的变化绑定在一起，以此来实现 View 和 Model 的同步更新。这样就==实现了对 View 和 Model 的解耦==，Presenter 还包含了其他的响应逻辑。  
...
> [!NOTE]
> - computed 计算属性 : 依赖其它属性值，并且 computed 的值有缓存，只有它依赖的属性值发生改变，下一次获取 computed 的值时才会重新计算 computed 的值，不支持异步。
...
除此之外，在Vue中，还可以是用==keep-alive来缓存页面，当组件在keep-alive内被切换时组件的**activated、deactivated**这两个生命周期钩子函数会被执行==  
...
缓存的是 组件实例对象（VNode），包括其 data 数据、DOM 状态、生命周期等。
也就是说：
- data() 中的数据保留；
- 页面滚动、输入框内容、切换状态等保留；
- 不会重新执行 created / mounted，只会触发 activated / deactivated 生命周期钩子。
📦 实现原理简要说明

<keep-alive> 的核心思想是：
1. 首次渲染时，将组件正常渲染；
2. 后续切换时，不是销毁旧组件，而是将其 缓存进一个 LRU Map；
3. 再次需要该组件时，从缓存中取出（而不是重新创建）；
4. 如果有 max 限制，超出数量的组件会被销毁并从缓存中剔除。
Vue 内部会给缓存的组件打标签，使用组件名（name）作为 key 进行缓存管理

![Pasted image 20250601112542.png](/img/user/Pasted%20image%2020250601112542.png)
...
![](/img/user/前端八股/Vue/Attachments/1664434791727833.png)  
...
![](/img/user/前端八股/Vue/Attachments/1664434858289821.jpeg)  
...
![68747470733a2f2f63646e2e6e6c61726b2e636f6d2f79757175652f302f323032312f6a7065672f313530303630342f313630393531383438303237322d38636231616630312d613461382d346435342d393162622d3535343661616661633531302e6a7065673f782d](/img/user/前端八股/Vue/Attachments/1664435356234568.jpg)  
...
补充：
## 前端路由（SPA 模式）与服务端渲染（SSR）

### 🔍 SPA（单页应用） vs SSR（服务端渲染）对比表

|对比项|SSR（服务端渲染）|SPA（单页应用）|
|---|---|---|
|**渲染方式**|服务端返回完整 HTML 页面|首次返回一个空壳 HTML，后续内容通过 JS 动态渲染|
|**路由控制**|路由由服务端控制（不同 URL 返回不同页面）|路由由前端框架（如 Vue Router、React Router）控制|
|**首屏加载速度**|首屏快（HTML 已生成）|首屏慢（需加载 JS 后再渲染）|
|**SEO 友好**|✅ 好，搜索引擎直接抓 HTML|❌ 差，需额外配置（如 prerender、SSR）|
|**用户体验**|页面跳转会刷新整个页面|页面跳转无刷新、流畅体验|
|**开发协同**|前后端混在一起，协作难|前后端分离，职责清晰|
|**服务端压力**|每次请求都需生成 HTML，服务器压力大|页面构建由前端负责，服务器压力小|
|**适合场景**|新闻类、博客、文档网站、重 SEO 场景|Web App、后台管理系统、互动型页面|
|**常见技术栈**|JSP、PHP、Node.js + EJS、Django 模板等|Vue + Vue Router、React + React Router + Axios 等|
|**优化方式**|可加缓存、CDN、Nginx 做负载均衡等|采用懒加载、骨架屏、预加载等前端优化策略|
...
- **现代开发趋势**：
    ...
    很多框架提供 SSR + SPA 混合模式（如 Nuxt.js、Next.js）兼顾 SEO 和用户体验。
    
- **面试技巧**：
    
    如果你被问到这个知识点，可以结合项目经验回答：
    
    > “我们项目采用了前端路由方式，实现了组件级页面跳转，提升了用户体验。如果有 SEO 需求，也可以通过 SSR 或预渲染等方式优化。”
### 36. Vue的性能优化有哪些
...
### 补充
#### 前端路由实现原理（Hash 模式 vs History 模式）
## 前端路由的实现方式

前端路由是指：在**不刷新页面的情况下**，根据 URL 的变化来控制页面内容的切换。主要有两种方式：
...
- URL 丑（有 `#`）；
- 对 SEO 支持差（搜索引擎通常不抓取 hash 后面的内容）；

### 🧪 技术实现（伪代码）：

```
window.addEventListener('hashchange', () => {
  const path = location.hash.slice(1); // 获取 # 后的路径
  renderPage(path);
});
...
### 2️⃣ History 路由模式（基于 `HTML5 History API`）

### 📌 原理

- 利用 `history.pushState()` 和 `popstate` 事件，来改变 URL 和监听跳转。
- 示例 URL: `http://example.com/home`
...
- 需要服务器配合配置（所有路径都需返回 `index.html`，避免 404）；
- 老旧浏览器不兼容（IE9 及以下）；

### 🧪 技术实现（伪代码）：

```
history.pushState({}, '', '/home'); // 改变路径但不刷新
...
|是否刷新页面|❌ 不刷新|❌ 不刷新|
|SEO 支持|❌ 差|✅ 可支持（需服务端配合）|
|是否需服务端支持|❌ 不需要|✅ 需要配置 nginx、Apache 等路由重定向|
|浏览器支持度|✅ 兼容性好（IE9+）|❌ 需现代浏览器（IE10+ 更稳定）|
|常见应用场景|简单项目、移动端网页、内嵌 H5 页面|企业级 SPA 项目、后台系统、需要 SEO 的页面|

---

## 📘 总结一句话

> Hash 模式依赖于浏览器对 # 的处理，简单好用但不支持 SEO；History 模式更美观、更高级，但部署时需要服务端支持。
...
> 具体来说，我总结了它们的几个主要区别：
> 
> 1. **语法和使用方式**：Vuex 强制使用 mutations 来修改 state，流程更严格，但也更繁琐；而 Pinia 更加简洁，允许在 actions 中直接修改 state，省去了中间的 mutation 步骤。
>     
> 2. **TypeScript 兼容性**：Pinia 从设计之初就考虑了 TS 支持，类型推导非常自然，写法也更贴合现代开发习惯，而 Vuex 在 TS 中使用时需要写大量的辅助类型。
>     
> 3. **与 Vue 3 的契合度**：Pinia 完全基于 Composition API 开发，能很好地和 `setup()` 一起使用；而 Vuex 在 Vue 3 中虽然可以用，但本身还是偏向 Vue 2 的 Options API 风格。
>     
> 4. **模块化管理**：Pinia 每个 store 都是一个独立模块，使用 `defineStore` 声明即可；相比之下，Vuex 的模块注册写法更复杂。
>     
> 5. **开发工具支持**：Pinia 在 Vue Devtools 里有更好的可视化体验，比如状态变化追踪、action 调用记录都更清晰。
>     
> 
> 所以如果是基于 Vue 3 的新项目，我更推荐直接使用 Pinia，不仅开发效率高，维护成本也更低。我在实际项目中也有使用 Pinia 的经验，整体感觉比 Vuex 更轻便灵活。
...
![](/img/user/前端八股/Vue/Attachments/1664438341498804.png)  
...
标识元素的身份，实现高效复用，在更新渲染时更加高效，准确key随元素移动不会产生顺序错误。 vue 中 key 值的作用可以分为两种情况来考虑：
