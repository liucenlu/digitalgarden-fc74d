---
{"dg-publish":true,"permalink":"/前端八股/性能优化/前端面试 性能优化篇_w3cschool/","created":"2025-05-27T15:55:40.000+08:00","updated":"2025-06-17T21:27:27.553+08:00"}
---
...
![](/img/user/前端八股/性能优化/Attachments/1663741895175671.png)
...
![image](./前端面试%20性能优化篇_w3cschool_files/errno-404)  
...
- **预加载指的是将所需的资源提前请求加载到本地，这样后面在需要用到时就直接从缓存取资源。**通过预加载能够减少用户的等待时间，提高用户的体验。我了解的预加载的最常用的方式是使用 js 中的 image 对象，通过为 image 对象来设置 scr 属性，来实现图片的预加载。
	- preload和prefetch的区别
 > `preload` 是 **当前页面** 需要的资源，提前加载；  
> `prefetch` 是 **未来页面** 可能用到的资源，空闲时加载。  
> 前者影响当前性能，后者为未来做准备。
-`preload` 是告诉浏览器“我马上就要用这个资源了”，请优先加载；而 `prefetch` 是告诉浏览器“我可能稍后会用到这个资源”，请在空闲时预加载，不会影响当前性能。两者都用于优化用户体验，但时机和优先级不同。
...
- **Tree Shaking**: 将代码中永远不会用到的片段删除掉。可以通过在启动webpack时追加参数 --optimize-minimize 来实现
- **Code Splitting**: 将代码按路由维度或者组件分块(chunk),这样做到按需加载,同时可以充分利用浏览器缓存
- **提取公共第三⽅库**: SplitChunksPlugin插件来进⾏公共模块抽取,利⽤浏览器缓存可以长期缓存这些⽆需频繁变动的公共代码
...
- **Code Splitting：** 把打包后的代码拆分成多个文件，按需加载，减少首屏加载体积。
    
- **Tree Shaking：** 移除项目中未被使用的代码，减少最终打包体积。
...
|特性|Code Splitting（代码分割）|Tree Shaking（摇树优化）|
|---|---|---|
|**核心作用**|按需加载模块，减少初始加载时间|去除未被使用的代码，减小打包体积|
|**目标**|提升加载性能|精简打包结果|
|**应用时机**|运行时加载需要的模块|构建时移除无用代码|
|**依赖条件**|Webpack/Rollup 支持动态 import()|ES Module 静态语法 + 构建工具支持|
|**常见场景**|路由懒加载、组件懒加载、第三方库拆分|未使用的工具函数、组件、库方法不被打包进产物|

---

#### 📖 详细讲解

##### ✂️ 1. Tree Shaking（摇树优化）

- **概念：**  
    构建工具在打包时分析模块的导出和引用，只打包实际“被用到”的部分，其余未使用的代码“像树叶一样被摇下来”移除掉。
    
- **前提条件：**
    
    - 必须使用 **ES6 模块语法**（`import` / `export`）
        
    - 构建工具支持（如 Webpack、Rollup）
...
- **注意：**
    ...
    - Tree Shaking 只适用于 **纯函数式代码**，有副作用（side effects）的代码不会被删除，除非设置 `sideEffects: false`
...
- **概念：**  
    把代码拆成多个 bundle（文件），在用户需要某个模块的时候才加载它，**提升首屏加载速度**。
...
- **常见应用：**
    ...
    - 第三方库（如 ECharts、moment）按需加载
        
    - 管理员页面和用户页面使用不同 JS bundle
...
|---|---|
|**减小体积**|Tree Shaking|
|**减少首屏加载时间**|Code Splitting + 懒加载|

---

#### 🧠 面试官喜欢的回答总结：

> Tree Shaking 关注的是“去掉没用的代码”，而 Code Splitting 是“把代码拆成多个包，按需加载”。两者都能减小最终构建体积，但应用时机和目标不同。在真实项目中，我们常常**配合使用**：先用 Tree Shaking 去掉不用的代码，再用 Code Splitting 拆包优化加载性能。
