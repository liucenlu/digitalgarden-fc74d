---
{"dg-publish":true,"permalink":"/前端八股/Javascript/前端面试 JavaScript篇_w3cschool/","created":"2025-05-25T15:27:40.460+08:00","updated":"2025-06-17T21:37:18.560+08:00"}
---
...
- **`Object.prototype.toString` 的用途**：它能够准确地识别各种数据类型，不管是基本数据类型还是引用数据类型。
...
![](/img/user/前端八股/Javascript/Attachments/20250526203609.png)
...
- 对于**单字节**的符号，字节的第一位为0，后面的7位为这个字符的 Unicode编码，因此对于英文字母，它的 Unicode编码和 ACSII编码一样。
...
- 第三种方式，如果需要判断的是某个内置的引用类型的话，可以使用 Object.prototype.toString() 方法来打印对象的[[Class\|Class]] 属性来进行判断。
...
> [!NOTE]
> 总结：prototype是每一个构造函数的内部属性，是一个对象。如果通过该构造函数进行了实例化，则该实例就会有一个隐藏属性 `__proto__`指向这个构造函数的prototype。（实例是孩子，构造函数是父母，孩子的 `__proto__`指向父母的prototype。通过 `__proto__` 指针链接起来的“继承路径”就是原型链）
...
p.sayHello(); // 查找 sayHello 时的过程：

```

##### 查找过程：

1. `p` 对象自身有没有 `sayHello`？→ ❌
    
2. 看 `p.__proto__` → 即 `Person.prototype` 上有没有？→ ✅ 找到并执行
    
3. 如果还没有，就继续看 `Person.prototype.__proto__`（即 Object.prototype）
...
![](/img/user/前端八股/Javascript/Attachments/1663558479535288.png)  
...
![](/img/user/前端八股/Javascript/Attachments/1663559513359628.jpeg)  
...
> 作用域是**变量/函数的可访问范围**，它决定了在哪些位置能访问某个变量或函数。
...
- 定义在**所有函数之外**的变量、函数、对象都属于全局作用域。
- 在**任何地方**都可以访问（包括函数内部）。
- 属于 `window`（浏览器）或 `global`（Node）对象的属性。

（2）函数作用域  

- 定义在**函数内部**的变量，只在该函数中可访问。
- 外部无法访问函数内部的变量。
- 函数内定义的变量不会污染外部作用域。
...
-  用 `{}` 包裹的代码块形成作用域（如 `if`, `for`, `while`, `{}` 等）
- 使用 `let` 和 `const` 声明的变量才具备块级作用域，`var` 仍是函数作用域
...
![img](/img/user/前端八股/Javascript/Attachments/2636768747470733a2f2f63646e2e6e6c61726b2e636f6d2f79757175652f302f323032302f706e672f313530303630342f313630353039393431313837332d64326561633235612d356438632d343538362d626333362d373639626365373.png)  
...
**内存泄露**是指程序在动态分配内存后，无法释放或者回收不再需要的内存空间的现象。通俗来说就是垃圾内存没有得到释放。
1. **未正确释放事件监听器**:如果一个对象被注册为某个事件的监听器，但在不需要它时未手动移除，这将导致该对象无法被垃圾回收。例如，当使用 addEventListener注册事件监听器时，需要使removeEventListener 除对应的监听器。
2. **定时器未清理**:使用 setTimeout、setinterval 创建的定时器，如果不及时清除，会导致函数或对象在定时器仍在运行时不能被垃圾回收。确保在不需要定时器时使用 clearTimeout 或 clearinterval 清除。
3. **闭包中的变量引用**:闭包是指函数可以访问并持有其词法环境中的变量。如果在闭包中引用了一些不再使用的变量，这些变量将无法被及时释放。解决方法是确保不再需要的变量解除引用或将其设置为null。
4. **脱离 DOM 的引用**:获取一个 DOM 元素的引用，而后面这个元素被删除，由于一直保留了对这个元素的引用，所以它也无法被回收。
5. **循环引用**:循环引用是指两个或多个对象相互引用，形成一个闭环，导致垃圾回收器无法判断其中哪个对象可以被释放。这通常发生在对象之间的相互引用，例如父子关系或对象之间的交互。解决方法是在不再需要引用时手动断开循环引用，例如将对象的引用设置为null。
6. **大量缓存数据**:如果程序中大量使用缓存，而没有有效地清理过期或不再使用的缓存数据，这些数据将继续占用内存。需要定期检查和清理不再需要的缓存数据。

## 十、补充
###  `arguments`对象

在 JavaScript 里，`arguments`对象是函数内部能够访问到的类数组对象。它的作用是存放函数调用时传入的实参。下面为你详细介绍它的特点和使用场景：
#### 主要特点

1. **类数组结构**：`arguments`对象具备`length`属性，而且可以通过索引访问元素，不过它并没有数组的方法，像`map`、`forEach`就无法使用。
2. **动态获取参数**：不管函数在定义时是否声明了参数，在函数被调用时，`arguments`对象都会包含所有实际传入的参数。
3. **没有自身方法**：由于`arguments`对象不是真正的数组，所以它不支持数组的原生方法。不过，我们可以借助`call`、`apply`或者`Array.from`来将其转换为数组。

#### 基本用法

下面通过一个简单的例子来说明`arguments`对象的用法：
...
需要注意的是，箭头函数没有自己的`arguments`对象。如果在箭头函数中使用`arguments`，它引用的其实是外层函数的`arguments`对象。例如：


```javascript
const multiply = () => {
    // 这里会报错，因为箭头函数没有arguments对象
    console.log(arguments); 
};

function outer() {
    const inner = () => {
        // 这里的arguments实际上是outer函数的arguments
...
为了能使用数组的方法，我们可以把`arguments`对象转换为数组。常见的转换方法有以下几种：
...
- **可变参数函数**：当函数需要处理不定数量的参数时，比如前面提到的求和函数。
- **函数重载模拟**：在 JavaScript 中没有函数重载的概念，我们可以通过`arguments`对象来模拟实现。
- **包装函数**：在实现一些工具函数，如`console.log`时，`arguments`对象就非常有用。

#### 替代方案

在现代 JavaScript 中，我们更推荐使用**剩余参数语法（Rest Parameters）**，它能直接把多余的参数收集到一个真正的数组中。示例如下：
...
`arguments`对象是 JavaScript 早期处理可变参数的一种方式，虽然现在有了更简洁的剩余参数语法，但在处理遗留代码或者需要兼容旧环境时，它仍然很有用。

### `prototype` 属性
当然，下面我将为你**详细讲解 JavaScript 中的 `prototype`（原型）机制**，并辅以图示结构和代码示例，帮助你彻底理解它在 JavaScript 中的地位和工作原理。
...
- 每个**函数**在创建时，都会自动拥有一个属性叫做 `prototype`。
    
- 这个 `prototype` 是一个对象，它的作用是**为该函数的实例提供共享属性和方法**。
    

通俗来说：

> `prototype` 就是函数创建的对象的“共享衣柜”，所有通过这个函数创建的对象，都可以访问这个“衣柜”中的内容。
...
- `Person.prototype` 是它的原型对象。
    
- `p1` 和 `p2` 都是用 `new Person()` 创建的实例，它们会自动拥有一个隐藏属性 `__proto__`，指向 `Person.prototype`。
...
当访问对象的属性或方法时，**会沿着原型链向上查找**，直到找到为止。

---

#### 四、prototype vs **proto**

|属性|描述|
|---|---|
|`prototype`|只有函数才有，是构造函数用于实例继承的模板|
...
> 所以 ES6 的 `class` 只是对 prototype 的**语法糖**，底层仍然使用原型机制。

---

#### 八、prototype 的小结图（结构图）
...
**Q1:** `prototype` 是对象吗？  
✅ 是，它是一个对象，默认有一个 `constructor` 属性。

**Q2:** `__proto__` 和 `prototype` 的区别？  
✅ `prototype` 是函数独有，`__proto__` 是对象的隐藏属性。

**Q3:** 怎么实现继承？  
✅ 构造函数继承 + 原型继承（如 `Object.create()`）是常用组合方式。
...
> 栈和队列都是常见的线性数据结构，它们的主要区别在于**数据的进出顺序不同**：
> 
> - **栈（Stack）**是**后进先出（LIFO）**的结构，最新放入的数据最先被取出。就像一叠盘子，最后放上去的盘子最先拿出来。
>     
> - **队列（Queue）**是**先进先出（FIFO）**的结构，最早进入的数据最先被取出，就像排队买票，先到的人先服务。
>     
> 
> 在实现上：
> 
> - 栈常用的方法是 `push()` 入栈，`pop()` 出栈。
>     
> - 队列常用的方法是 `enqueue()` 入队，`dequeue()` 出队。
>     
> 
> 它们的应用场景也不同：
> 
> - 栈适合用于**递归处理、括号匹配、撤销操作**等场景。
>     
> - 队列常用于**任务调度、消息排队、广度优先搜索（BFS）**等。
>     
> 
> 另外，队列还有一些变种，比如：
> 
> - **双端队列（Deque）**，两端都可以进出；
>     
> - **优先队列（Priority Queue）**，根据优先级出队，不一定是最先入队的元素先出。
>     

> 所以，总结来说，**核心区别就是操作顺序不同：栈是后进先出，队列是先进先出**，根据业务场景选择合适的数据结构会更高效。