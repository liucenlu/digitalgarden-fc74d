---
{"dg-publish":true,"permalink":"/03_Vue/Vue入门到实战/Vue入门到实战/","created":"2025-08-12T11:06:00.047+08:00","updated":"2025-10-09T18:57:45.768+08:00"}
---

[Vue入门到实战-个人笔记](Vue入门到实战-个人笔记.md)
[组件说明文档](组件说明文档.md)
# 1.前端框架的由来
...
```
# 3.核心概念
![](/img/user/03_Vue/attachments/Paste-image-20250813.png)
## 注入
...
在render中生成vnode（只为方便理解）
```js
//h:createVNode,第一个参数：tag，第二个参数：children
        render(h) {
          console.log("render");
          return h("div", [
...
```
![](/img/user/03_Vue/attachments/Paste-image-20250910.png)
# 7.组件事件
## pager组件
...
import preview from 'vite-plugin-vue-preview'
...
可以直接在浏览器中访问某个组件的预览路径
```
http://localhost:5173/__preview__/components/MyComponent.vue
```
# 9. 组件练习
[组件说明文档](组件说明文档.md)
# 10.插槽
...
3. abstract：路由从内存中获取路径，改变路径也只是改动内存中的值。这种模式通常应用到非浏览器环境中。(==在vue3对应Router4中被`Memory` 模式替代==)
...
function getComponentRootDom(comp, props) {
  ...
  // 返回组件渲染的根元素（container 的第一个子元素，即组件根节点）
...
Vue 3 中使用 `app.config.globalProperties` 来定义全局属性，所有组件实例都能访问到这些属性。(简单扩展用 **全局属性**，复杂场景用 **插件**)
...
// 扩展全局属性（类似 Vue 2 的 Vue.prototype）
...
const handleClick = () => { instance.appContext.config.globalProperties.$sayHello() } </script>
```

`getCurrentInstance` 是 Vue 3 提供的一个内置 API，用于在 **组件内部** 获取当前组件实例的相关信息
## ref
...
```
![](/img/user/03_Vue/Vue入门到实战/attachments/Paste-image-20250928.png)
